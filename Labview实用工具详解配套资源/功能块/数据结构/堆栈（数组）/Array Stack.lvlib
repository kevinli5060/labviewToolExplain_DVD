<?xml version='1.0' encoding='UTF-8'?>
<Library LVVersion="12008004">
	<Property Name="NI.Lib.Icon" Type="Bin">%A#!"!!!!!)!"1!&amp;!!!-!%!!!@````]!!!!"!!%!!!(^!!!*Q(C=\&gt;8"&lt;2MR%!813:"$A*T51;!7JA7VI";G"6V^6!P4AFJ1#^/#7F!,TN/'-(++=IC2(-TVS+O`80+:3[QDNP9VYEO]0GP@@NM_LD_\`K4&amp;2`NI`\;^0.WE\\ZH0]8D2;2'N3K6]:DK&gt;?1D(`H)2T\SFL?]Z3VP?=N,8P+3F\TE*5^ZSF/?]J3H@$PE)1^ZS*('Z'/C-?A99(2'C@%R0--T0-0D;QT0]!T0]!S0,D%]QT-]QT-]&lt;IPB':\B':\B-&gt;1GG?W1]QS0Y;.ZGK&gt;ZGK&gt;Z4"H.UQ"NMD:Q'Q1DWM6WUDT.UTR/IXG;JXG;JXF=DO:JHO:JHO:RS\9KP7E?BZT(-&amp;%]R6-]R6-]BI\C+:\C+:\C-6U54`%52*GQ$)Y1Z;&lt;3I8QJHO,R+YKH?)KH?)L(J?U*V&lt;9S$]XDE0-E4`)E4`)EDS%C?:)H?:)H?1Q&lt;S:-]S:-]S7/K3*\E3:Y%3:/;0N*A[=&lt;5+18*YW@&lt;,&lt;E^J&gt;YEO2U2;`0'WJ3R.FOM422L=]2[[,%?:KS(&amp;'PR9SVKL-7+N1CR`LB9[&amp;C97*0%OPH2-?Y_&lt;_KK,OKM4OKI$GKP&gt;I^&lt;`X,(_`U?N^MNLN&gt;L8#[8/*`0=4K&gt;YHA]RO&amp;QC0V_(\P&gt;\OUV].XR^E,Y_6Z[=@YH^5\`3`_$&gt;W.]DF`(N59`!/&lt;!-PQ!!!!!</Property>
	<Property Name="NI.Lib.SourceVersion" Type="Int">302022660</Property>
	<Property Name="NI.Lib.Version" Type="Str">1.0.0.0</Property>
	<Item Name="Custom Def" Type="Folder">
		<Item Name="堆栈操作.ctl" Type="VI" URL="../堆栈操作.ctl"/>
	</Item>
	<Item Name="DEMO" Type="Folder">
		<Item Name="DEMO.vi" Type="VI" URL="../DEMO.vi"/>
	</Item>
	<Item Name="Interface" Type="Folder">
		<Item Name="堆栈（数组）.vi" Type="VI" URL="../堆栈（数组）.vi"/>
	</Item>
	<Item Name="VI" Type="Folder">
		<Item Name="堆栈（CDB）.vi" Type="VI" URL="../堆栈（CDB）.vi"/>
		<Item Name="堆栈（CSG）.vi" Type="VI" URL="../堆栈（CSG）.vi"/>
		<Item Name="堆栈（DBL）.vi" Type="VI" URL="../堆栈（DBL）.vi"/>
		<Item Name="堆栈（I8）.vi" Type="VI" URL="../堆栈（I8）.vi"/>
		<Item Name="堆栈（I16）.vi" Type="VI" URL="../堆栈（I16）.vi"/>
		<Item Name="堆栈（I32）.vi" Type="VI" URL="../堆栈（I32）.vi"/>
		<Item Name="堆栈（Path）.vi" Type="VI" URL="../堆栈（Path）.vi"/>
		<Item Name="堆栈（SGL）.vi" Type="VI" URL="../堆栈（SGL）.vi"/>
		<Item Name="堆栈（String）.vi" Type="VI" URL="../堆栈（String）.vi"/>
		<Item Name="堆栈（U8）.vi" Type="VI" URL="../堆栈（U8）.vi"/>
		<Item Name="堆栈（U16）.vi" Type="VI" URL="../堆栈（U16）.vi"/>
		<Item Name="堆栈（U32）.vi" Type="VI" URL="../堆栈（U32）.vi"/>
		<Item Name="堆栈（Variant）.vi" Type="VI" URL="../堆栈（Variant）.vi"/>
		<Item Name="堆栈（布尔）.vi" Type="VI" URL="../堆栈（布尔）.vi"/>
	</Item>
</Library>
